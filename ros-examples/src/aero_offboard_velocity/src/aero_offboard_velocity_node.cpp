...
void saveCurrentStateCb(const mavros_msgs::StateConstPtr &msg,
...
void waitForFCUConnection() {
  ...
  ros::Subscriber flight_mode_sub = node->subscribe<mavros_msgs::State>(
      "mavros/state", 3, boost::bind(saveCurrentStateCb, _1, &current_state));
...
float Offboard::convertToRadFromDeg(float deg) { return (float)(deg / 180.0f * M_PI); }

Offboard::Offboard(ros::NodeHandle *nodehandle) {
  nh_ = *nodehandle;
  set_vel_pub = nh_.advertise<mavros_msgs::PositionTarget>(
      "mavros/setpoint_raw/local", 10);
}

void Offboard::setOffboardVelocityBody(float vx, float vy, float vz,
                                       float yaw_rate) {
